...
export const transitionUniformNames=["gridSize","gridOrigin","gridDirection","gridFront","gridFeather","transitionKind","transitionProgress","pinwheelFronts","dissolveSeed","radialCenter","radialCurvature","radialInset"];
export class BabylonTransitionUniforms {
  ...
  update(r:BabylonSceneContext,m:ShaderMaterial,c:ComponentMap["Transition"]|undefined,bounds:Bounds|null):void {
    ...
    if(c.kind==="grid"){
      ...
      const g=c.radialGrid;r.vec2(m,"gridSize",g.cellSize);r.vec2(m,"gridOrigin",g.origin);r.vec2(m,"radialCenter",g.center);m.setFloat("radialCurvature",g.curvature);m.setFloat("radialInset",g.inset);
