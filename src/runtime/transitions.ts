...
import type {Bounds,ComponentMap,Matrix,View} from "./types.js";
...
export function transitionGroups(scene:Scene,view:View):TransitionGroup[]{
  ...
  for(const node of scene.nodes.values()){
    ...
    groups.push({id:node.id,component:c,members,depth:viewMatrix[14],viewMatrix,bounds:scene.coverage(node.id,view,.1),frameBounds:scene.coverage(node.id,view,0)});
  }
