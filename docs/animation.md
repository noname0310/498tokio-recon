...
Object references use either `{entity: "scene-id"}` or `{binding: "slot-id", descendant: "optional-original-child-id"}`. A possessable targets an existing object. A spawnable supplies an `EntityInput` template, an optional parent reference and an optional Boolean `spawnTrack` ID. Without a spawn track it exists throughout its active sequence. Spawn IDs are namespaced per sequence instance; template IDs can match authored IDs. Spawned entities inherit the specified local hierarchy. Missing spawned parents suppress their children, and exiting a sequence removes its spawnables. Both renderers reconcile this set incrementally and dispose the removed DOM/GPU objects.
...
`PerformanceClock` measures elapsed microseconds from a rational absolute anchor and does not accumulate frame deltas. Exact frame seeks and reverse/zero rates are supported. With an audio clock, `seekFrame` converts the requested frame to media seconds, then the pose follows the browser's actual media position; native Audio does not promise sample-accurate seeks. Legacy SpriteAnimator/TransformAnimator data still plays from the same selected clock. Avoid writing the same sprite frame with both SpriteAnimator and a new track: the legacy atlas animator runs after the frame property is sampled. Legacy TransformAnimator provides the base transform beneath new track overrides.
...
Run `npm run check` to build production bundles and validate type contracts, frame arithmetic, mixed/weighted interpolation, nested rates, loops, restoration, graph rejection and both browser backends. Browser checks verify no Canvas in DOM, no global Babylon script, lazy loading, exact rewind results, spawn resource counts, pause/reverse/half-speed playback and existing scene baselines.
