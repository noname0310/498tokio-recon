...
`FrameTime` contains `{frame: FrameNumber, subframe: {numerator: number, denominator: number}}`. Fractions use checked safe integers and normalize to `[0,1)`, including negative times. Whole frames stay separate, products cancel common factors first, and fraction comparisons avoid cross multiplication. Integer seeks, frame-rate conversions, sequence offsets and loops therefore retain exact rational time within this range. Unsupported integer overflow throws. Curve evaluation subtracts whole key times before converting the remaining fraction to floating point.

`Time.fromRatio` and `Time.fromParts` construct exact times. `Time.fromDecimal` samples floating input at a resolution of one billionth of a frame; `Time.fromSeconds` uses the same resolution in seconds before rate conversion. Inputs with at most nine decimal places remain exact. Longer fractions truncate toward the previous sampling instant and never snap across a whole-frame key. This finite precision belongs to the floating-input boundary, not to animation keys or exact frame seeks. Browser clock samples and fitted fractional particle emission rates use this boundary; PerformanceClock continues sampling wall time in integer microseconds.
...
await engine.seekFrame(Time.fromRatio(115, 2));       // 57.5 display frames
...
The side-flight sequence resumes at frame 2686 with five beams and a two-frame rocket atlas. Its camera blur decreases from sigma 11.03 reference pixels to zero at frame 2692. Two weighted Bezier moves, a hold and an offscreen continuation use five ship-position keys. Seventeen tracked rockets depart one nozzle attached to the ship, move backward briefly, then accelerate forward while their vertical velocity decays. Sparse bursts store inferred birth times and velocity pairs rather than per-frame positions. Occluded launch instants are inferred; the later offscreen size bands retain statistical distributions. The outgoing image fades during frames 2760–2767. The incoming 3D shot is not yet authored.

`ParticleEmitter.velocityRelaxation` optionally supplies per-axis decay `rate` (inverse seconds), `target` velocity and seeded `variation` around that target. For each axis, `p(t) = p0 + target*t + (initial-target)*(1-exp(-rate*t))/rate`; a zero rate keeps the initial velocity. Burst `velocity` and `targetVelocity` override the initial and target vectors. Additional acceleration remains additive. Motion and its derivative are evaluated directly at age, so seeking, local parenting and motion blur remain deterministic. Relaxation cannot combine with `speedOverLife` or straight-stream camera continuation.

Foreground rockets use the existing particle filter's isotropic softness with a zero motion shutter: sigma 0.215 native cells, about 1.7 pixels at the reference size. DOM retains an SVG Gaussian filter, and Babylon reuses a filtered atlas across all instances and animation frames. Smaller rockets share the original sharp atlas.

`TiledSpriteRenderer.brightness` defaults to one and multiplies RGB after saturation. It adjusts exposure without duplicating the shared tile. `ParticleEmitter.animation.timeSource` defaults to `"age"`; `"scene"` selects atlas frames from the absolute scene clock so particles born at different times can blink together. The rocket size bands use reversed frame lists for opposite phases. `npm run check:evasion` checks measured poses, exact cuts and fade timing, synchronized versus age-based atlas playback, tile brightness, viewport expansion and deterministic replay in both renderers.

The returning paper plane reuses the original sprite at five reference pixels per texel. A two-key linear path moves it left at 29.7844 pixels per source frame. Frames 3424–3430 reveal the reused landscape through a radial grid field; three weighted Bezier keys each animate progress and curvature. The first five visible contours have a fitted RMS residual of 0.098 reference pixels. The final subpixel dots use an area measurement because their centers no longer reach half opacity. These are sparse fitted curves, not per-frame tile animation.

The rear ship in the return flight uses `ship_rear_red`. Its 29 changed canopy texels are sampled from registered, unoccluded observations around frame 3095. The user-edited hull, golden highlight and alpha mask are shared with the blue reference; the earlier forward flight continues to use that blue asset.
