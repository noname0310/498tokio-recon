...
async function waitForPlayer(page,label){
 try{
  await page.waitForFunction(()=>window.scenePlayer?.ready||document.getElementById('status')?.getAttribute('role')==='alert');
  const failure=await page.evaluate(()=>{
   const status=document.getElementById('status');
   return status?.getAttribute('role')==='alert'?(status.textContent||'Player initialization failed'):null;
  });
...
async function captureFrame(page){
 // Headed Firefox can defer presentation of an inactive tab after a seek.
 await page.bringToFront();
 await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
 return page.screenshot({style:'.runtime-loading-status { visibility: hidden !important; }'});
}
async function main(){
 ...
 try{for(const [name,type]of [['chromium',chromium],['firefox',firefox]]){
  // Linux Firefox needs an X server for WebGL; CI supplies it through Xvfb.
  const browser=await type.launch({headless:!(name==='firefox'&&process.platform==='linux')});
  try{
   ...
   for(const frame of [15,173,540,900,1150,1500,1943,2200,2225,2245,4088,4091,4194,4245,4422,4533,4665,4800,4867,4940]){
   ...
   await file.bringToFront();await file.evaluate(async()=>{await scenePlayer.seekFrame(360,{numerator:30,denominator:1});await scenePlayer.play();});await file.waitForFunction(()=>scenePlayer.time>12.15);await file.evaluate(()=>scenePlayer.pause());
   ...
   await assert.rejects(waitForPlayer(unsupported,`${name} unsupported WebGL`),/Could not display the scene:.*WebGL/i);
