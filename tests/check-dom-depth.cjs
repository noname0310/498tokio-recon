...
async function main(){
  ...
  try{for(const [name,type] of [["chromium",chromium],["firefox",firefox]]){
    ...
    try{
      ...
      await page.addInitScript(()=>{HTMLCanvasElement.prototype.getContext=()=>{throw new Error("DOM canvas forbidden");};window.OffscreenCanvas=class{constructor(){throw new Error("DOM OffscreenCanvas forbidden");}};});
      ...
      const cold=await page.screenshot();assert(bright(cold)>10,"The first flame must be visible immediately after loading");
      for(const frame of [8,45,173,481,542,547,558]){
        await sample(frame);const image=await page.screenshot();
        ...
        if(frame===558){
          assert.equal(await page.evaluate(()=>scenePlayer.scene.active.get("warp-beam-0")),false,"The warp has ended");
          assert.equal(await page.evaluate(()=>scenePlayer.scene.active.get("starfield-scene")),true,"The incoming star field remains visible");
          assert(bright(image)>50,"Stars render over the black background after the beams end");
        }
      }
      await sample(8);const front=await page.screenshot();
      await page.evaluate(()=>scenePlayer.setTransform("reveal-curtain",{localPosition:{z:1}}));assert.equal(bright(await page.screenshot()),0,"Moving the curtain in front must hide the flames and their glow");
      await page.evaluate(()=>scenePlayer.setTransform("reveal-curtain",{localPosition:{z:3}}));close(await page.screenshot(),front,"Restoring depth restores the flame");
      await page.evaluate(()=>{const root=scenePlayer.renderer.world;for(const e of [...root.children].reverse())root.append(e);});close(await page.screenshot(),front,"Depth order is independent of DOM insertion order");
      await sample(0);close(await page.screenshot(),cold,"Rewinding restores the first flame");
      await page.evaluate(async()=>{await scenePlayer.seek(0);scenePlayer.setPlaybackRate(1,4);await scenePlayer.play();});
      await page.waitForFunction(()=>scenePlayer.time>.1);assert(bright(await page.screenshot())>10,"Flames remain visible during audio-clock playback");await page.evaluate(()=>scenePlayer.pause());
      await page.evaluate(data=>scenePlayer.loadScene(data),fixture());
      const image=await page.screenshot();assert.deepEqual(pixel(image,320,180),[255,0,0],"The nearer beam covers the later-created backdrop");
      ...
      assert.deepEqual(pixel(await page.screenshot(),320,180),[13,26,51],"The opaque backdrop covers the beam and glow after crossing depth");
      await page.evaluate(()=>scenePlayer.setTransform("camera",{localPosition:{z:10},localRotation:{y:180}}));
      assert.deepEqual(pixel(await page.screenshot(),320,180),[255,0,0],"Camera-space ordering reverses when the camera turns around");
