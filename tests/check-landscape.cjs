...
async function main(){
  ...
  for(const n of [1219,1220,1680,1219,1400]){at(n);assert.equal(scene.active.get('landscape-scene'),n>=1220);if(n>=1220)assert.equal(scene.active.get('paper-scene'),false);}
  ...
  for(const n of [1470,1500,1680,1726,1219,1386]){
    ...
    tiles.forEach((id,i)=>assert.equal(scene.world.get(id)[12],floorPositions[i],'Floor remains stationary in world space'));
  ...
  for(const [index,frame]of [[8,1669],[7,1682],[6,1696],[5,1709]]){
    for(const n of [frame-1,frame,frame+1,frame-1]){at(n);assert.equal(scene.active.get(`landscape-runner-${index}`),n<frame,`Runner ${index} disappears as a whole sprite at ${frame}, including reverse seeks`);}
  }
  ...
  at(1726);const ending=landscapePose();at(1800);assert.equal(landscapePose(),ending,'Retired landscape retains its last pose');
  assert.equal(scene.active.get('landscape-scene'),false,'The new camera cut retires the landscape hierarchy');
  ...
  try{for(const [name,type,renderer] of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox',firefox,'dom']]){
    ...
    try{
      ...
      for(const viewport of [{width:390,height:844},{width:1280,height:360}]){
        ...
        await page.evaluate(()=>scenePlayer.whenIdle());await page.screenshot({path:path.join(output,`${name}_${viewport.width}.png`)});
      }
      const flightOutput=path.join(output,'paper_flight');fs.mkdirSync(flightOutput,{recursive:true});
      for(const n of [1470,1480,1500]){
        await page.evaluate(async n=>{const {Frame}=await import('/runtime/player.js');await scenePlayer.seekFrame(Frame.from(n));await scenePlayer.whenIdle();},n);
        await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
        await page.screenshot({path:path.join(flightOutput,`${name}_after_${n}.png`)});
      }
      ...
      for(const viewport of [{width:640,height:360},{width:390,height:844},{width:1280,height:360}]){
        ...
        for(const mode of ['repeatBottom','repeat'])for(const position of [{x:.137,y:-.321},{x:-7.431,y:.179}]){
          ...
          checkRepeat(await page.screenshot(),view,asset,mode,position);
        }
      }
      if(renderer==='dom'){
