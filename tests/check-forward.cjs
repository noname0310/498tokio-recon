...
async function main(){
  ...
  for(const n of [846,847,850,851,1109,1117,1154,1155]){at(n);assert.equal(scene.isActive('forward-scene'),n>=851&&n<1155);}
  at(900);
    ...
    for(const [label,type,renderer] of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
      ...
      try{
        ...
        for(const n of [847,851,900,960,1109,1117,1144,1147,1149,1152,1155,900]){
          await seek(n);const capture=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }",path:path.join(output,`${label}_${n}.png`)});
        ...
        for(const viewport of [{width:375,height:812},{width:1400,height:600}]){await page.setViewportSize(viewport);await seek(1155);const im=png(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));for(const [x,y] of [[0,0],[viewport.width-1,viewport.height-1]])assert(im.pixels[(y*im.width+x)*im.channels+1]>170);}
        ...
        await page.evaluate(async data=>{await scenePlayer.loadScene(data);await scenePlayer.seek(.5);await scenePlayer.whenIdle();},fixture);
        const sharp=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));await page.evaluate(()=>scenePlayer.setComponent('particle','ParticleMotionBlur',{enabled:true}));
        const blur=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }",path:path.join(output,`${label}_blur_fixture.png`)}));
        ...
        const thick=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));
        ...
        const soft=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));
        ...
        const faint=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));assert(Math.abs(faint.mass/soft.mass-.5)<.02,'Alpha gain is applied after convolution');
        ...
        const faded=png(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));assert(faded.pixels.reduce((maximum,value,i)=>i%faded.channels===0?Math.max(maximum,value):maximum,0)<=52,'Particle opacity remains outside the saturating filter alpha gain');
        ...
        const disabled=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));assert(Math.abs(disabled.mass/sharp.mass-1)<.01,'Disabling the component restores the unfiltered sprite');
        ...
        const clippedTail=moments(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));assert(clippedTail.mass>1&&clippedTail.mass<sharp.mass/2,'Screen culling retains the visible blur tail of an offscreen sprite');
        ...
        await page.evaluate(async data=>{await scenePlayer.loadScene(data);await scenePlayer.seek(1);await scenePlayer.whenIdle();},sortedFixture);
        const sortedCapture=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}),sortedPixels=png(sortedCapture),center=(180*sortedPixels.width+320)*sortedPixels.channels;
        ...
        assert(compare(sortedCapture,await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"})).max<=1,'Size sorting is deterministic after reverse seek');
