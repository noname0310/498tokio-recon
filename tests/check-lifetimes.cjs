...
async function main(){
 ...
 for(const n of probes.concat(probes.toReversed())){
  ...
  assert.equal(scene.authoredNodes.size,15);for(const id of scene.nodes.keys())assert(scene.parents.get(id)===null||scene.nodes.has(scene.parents.get(id).id));
 }
 at(4050);assert.equal(scene.nodes.size,15);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 console.log('Lifetimes: exact boundaries, fifteen resident entities, nested room ownership, ID scopes and prefab references passed.');
 ...
 try{for(const [label,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
  ...
  try{
   ...
   for(const n of [558,600,900,1150,1500,1943,2189,2200,2245,2256,2390,2536,2537,2570,2610,2611,2646,2685,2686,2716,2746,2767,2768,2878,2974,3034,3035,3132,3218,3295,3370,3400,3426,3430,3431,3504,3524,3603,3708,3777,3930,3940,4000,4078,4194,4245,4422,4535,4536,4050])await page.evaluate(async n=>{await scenePlayer.seekFrame(n,{numerator:30,denominator:1});await scenePlayer.whenIdle();},n);
   assert.deepEqual(await page.evaluate(()=>({nodes:scenePlayer.scene.nodes.size,objects:scenePlayer.renderer.objects.length,disposed:window.retained.object.disposed,cache:scenePlayer.resources===window.retained.resources&&scenePlayer.resources.images.size===window.retained.images,clock:scenePlayer.clock===window.retained.clock,camera:scenePlayer.scene.find('camera')===window.retained.camera,workers:window.workerCount})),{nodes:15,objects:0,disposed:true,cache:true,clock:true,camera:true,workers:1});
