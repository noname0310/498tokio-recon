...
async function main(){
 ...
 const ranges=[['intro-content',0,559],['starfield-content',548,900],['forward-scene',851,1156],['paper-content',1145,1220],['landscape-scene',1220,1727],['landscape-camera-layers',1220,1727],['interior-scene',1727,2196],['helmet-launch-scene',2196,2256]];
 ...
 at(2256);assert.equal(scene.nodes.size,7);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 try{for(const [label,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
  ...
  try{
   ...
   for(const n of [558,600,900,1150,1500,1943,2189,2200,2245,2256])await page.evaluate(async n=>{await scenePlayer.seekFrame(n,{numerator:30,denominator:1});await scenePlayer.whenIdle();},n);
