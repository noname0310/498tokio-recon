...
async function main(){
 ...
 for(const n of probes.concat(probes.toReversed())){
  ...
  assert.equal(scene.authoredNodes.size,16);for(const id of scene.nodes.keys())assert(scene.parents.get(id)===null||scene.nodes.has(scene.parents.get(id).id));
 }
 at(4050);assert.equal(scene.nodes.size,16);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 console.log('Lifetimes: exact boundaries, sixteen resident entities, nested room ownership, ID scopes and prefab references passed.');
