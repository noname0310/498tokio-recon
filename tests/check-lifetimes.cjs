...
async function main(){
 ...
 assert.deepEqual([...scene.authoredNodes.keys()].sort(),['final-animation','camera','landscape-camera','interior-camera','helmet-bars-camera','helmet-flat-camera','deck-camera','boss-camera','playback'].sort());
 const ranges=[['intro-content',0,559],['starfield-content',548,900],['forward-scene',851,1156],['paper-content',1145,1220],['landscape-scene',1220,1727],['landscape-camera-layers',1220,1727],['interior-scene',1727,2196],['helmet-launch-scene',2196,2256],['asteroid-scene',2256,2537],['deck-scene',2537,2611],['deck-background',2537,2611],['boss-scene',2611,2686]];
 ...
 for(const n of probes.concat(probes.toReversed())){
  ...
  assert.equal(scene.authoredNodes.size,9);for(const id of scene.nodes.keys())assert(scene.parents.get(id)===null||scene.nodes.has(scene.parents.get(id).id));
 }
 at(2686);assert.equal(scene.nodes.size,9);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 console.log('Lifetimes: exact boundaries, nine resident entities, nested room ownership, ID scopes and prefab references passed.');
 ...
 try{for(const [label,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
  ...
  try{
   ...
   for(const n of [558,600,900,1150,1500,1943,2189,2200,2245,2256,2390,2536,2537,2570,2610,2611,2646,2685,2686])await page.evaluate(async n=>{await scenePlayer.seekFrame(n,{numerator:30,denominator:1});await scenePlayer.whenIdle();},n);
   assert.deepEqual(await page.evaluate(()=>({nodes:scenePlayer.scene.nodes.size,objects:scenePlayer.renderer.objects.length,disposed:window.retained.object.disposed,cache:scenePlayer.resources===window.retained.resources&&scenePlayer.resources.images.size===window.retained.images,clock:scenePlayer.clock===window.retained.clock,camera:scenePlayer.scene.find('camera')===window.retained.camera,workers:window.workerCount})),{nodes:9,objects:0,disposed:true,cache:true,clock:true,camera:true,workers:1});
