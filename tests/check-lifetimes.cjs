...
async function main(){
 ...
 at(3431);assert.equal(scene.nodes.size,14);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 try{for(const [label,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
  ...
  try{
   ...
   for(const n of [558,600,900,1150,1500,1943,2189,2200,2245,2256,2390,2536,2537,2570,2610,2611,2646,2685,2686,2716,2746,2767,2768,2878,2974,3034,3035,3132,3218,3295,3370,3400,3426,3430,3431])await page.evaluate(async n=>{await scenePlayer.seekFrame(n,{numerator:30,denominator:1});await scenePlayer.whenIdle();},n);
