...
async function main(){
 ...
 assert.deepEqual([...scene.authoredNodes.keys()].sort(),['final-animation','camera','landscape-camera','interior-camera','helmet-bars-camera','helmet-flat-camera','deck-camera','boss-camera','playback','destruction-camera-rig','destruction-camera-tilt','destruction-camera-orbit','destruction-camera','return-camera','biome-camera','landing-camera','storm-camera','ending-camera'].sort());
 const ranges=[['intro-content',0,559],['starfield-content',548,900],['forward-scene',851,1156],['paper-content',1145,1220],['landscape-scene',1220,1727],['landscape-camera-layers',1220,1727],['interior-scene',1727,2196],['helmet-launch-scene',2196,2256],['asteroid-scene',2256,2537],['deck-scene',2537,2611],['deck-background',2537,2611],['boss-scene',2611,2686],['evasion-scene',2686,2768],['destruction-scene',2761,3035],['return-moon-flight-root',3035,3134],['return-side-flight-root',3128,3370],['paper-return-flight-root',3370,3424],['paper-return-landscape-root',3424,3431],['paper-return-camera-layers',3424,3431],['flashback-children-root',3431,3708],['flashback-children-camera-layers',3431,3708],['flashback-meeting-root',3708,4042],['flashback-meeting-camera-layers',3708,4042],['biome-world',4078,4536],['landing-world',4530,4941],['storm-world',5532,5978],['ending-city-flight-world',5978,6123],['ending-departure-world',6123,6416],['ending-credits-world',6416,6711],['ending-thanks-world',6711,6901]];
 ...
 for(const n of probes.concat(probes.toReversed())){
  ...
  assert.equal(scene.authoredNodes.size,18);for(const id of scene.nodes.keys())assert(scene.parents.get(id)===null||scene.nodes.has(scene.parents.get(id).id));
 }
 at(4050);assert.equal(scene.nodes.size,18);assert.equal(scene.isActive('helmet-launch-hull'),false);
 ...
 console.log('Lifetimes: exact boundaries, eighteen resident entities, nested room ownership, ID scopes and prefab references passed.');
 ...
 try{for(const [label,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox-dom',firefox,'dom']]){
  ...
  try{
   ...
   for(const n of [558,600,900,1150,1500,1943,2189,2200,2245,2256,2390,2536,2537,2570,2610,2611,2646,2685,2686,2716,2746,2767,2768,2878,2974,3034,3035,3132,3218,3295,3370,3400,3426,3430,3431,3504,3524,3603,3708,3777,3930,3940,4000,4078,4194,4245,4422,4535,4536,4665,4697,4793,4800,4867,4940,4941,4050])await page.evaluate(async n=>{await scenePlayer.seekFrame(n,{numerator:30,denominator:1});await scenePlayer.whenIdle();},n);
   assert.deepEqual(await page.evaluate(()=>({nodes:scenePlayer.scene.nodes.size,objects:scenePlayer.renderer.objects.length,disposed:window.retained.object.disposed,cache:scenePlayer.resources===window.retained.resources&&scenePlayer.resources.images.size===window.retained.images,clock:scenePlayer.clock===window.retained.clock,camera:scenePlayer.scene.find('camera')===window.retained.camera,workers:window.workerCount})),{nodes:18,objects:0,disposed:true,cache:true,clock:true,camera:true,workers:1});
