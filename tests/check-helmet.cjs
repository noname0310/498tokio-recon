...
async function main(){
 ...
 const future=[...new Set([...scene.nodes.keys(),...scene.sequence.templates.keys()])].filter(id=>id.startsWith('helmet-'));
 for(const n of [0,1735,2083,2098,2100]){at(n);for(const id of future)assert(!scene.isActive(id),`${id} leaked into frame ${n}`);}
 for(const [id,frame] of [['helmet-falling',2119],['helmet-worn',2157],['helmet-closeup',2220]]){at(frame);assert.equal(scene.requireComponent(id,'SpriteRenderer').asset,'helmet_helmet_blue');}
 ...
 at(2255);assert(scene.isActive('helmet-launch-warp-0'));at(2256);assert(!scene.isActive('helmet-launch-warp-0'));
 ...
 at(2181);assert(scene.isActive('helmet-worn'));assert(scene.transformAt('helmet-worn').localScale.y>100);
 at(2182);assert(!scene.isActive('helmet-worn'));assert(!scene.isActive('interior-final-person'));
 ...
 at(2100);for(const id of future)assert(!scene.isActive(id),`${id} survived a reverse seek before its sequence`);
 ...
 try{for(const [name,type,renderer]of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox',firefox,'dom']]){
  ...
  try{const page=await browser.newPage({viewport:{width:640,height:360}}),errors=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error')errors.push(m.text());});
   ...
   for(const n of [2119,2157,2179,2180,2181,2182,2189,2190,2192,2194,2196,2200,2207,2208,2208.25,2216,2217,2224,2225,2226,2232,2235,2236,2238,2244,2245,2248,2255,2256,2100]){await seek(n);await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }",path:path.join(out,`${name}_${String(n).replace('.','_')}.png`)});}
   ...
   for(const n of [2224.999,2225,2235.999,2236,2244.999,2245,2236,2225,2224]){
    await seek(n);
    const state=await page.evaluate(()=>['helmet-closeup','helmet-face','helmet-launch-hull','helmet-launch-warp-0'].map(id=>scenePlayer.scene.isActive(id)));
    assert.deepEqual(state,[n<2225,n>=2225&&n<2235,n>=2236&&n<2245,n>=2245],'Appearance boundaries survive forward and reverse seeks');
   }
   await seek(2208);const first=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"});await seek(2216);await seek(2208);assert.equal(compare(first,await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"})).max,0,'Shrinking the digit count reuses and hides surplus slots');
   ...
   for(const size of [{width:390,height:844},{width:1280,height:320}]){
    await page.setViewportSize(size);for(const n of [2196,2207,2208.25,2226,2255]){await seek(n);await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }",path:path.join(out,`${name}_${String(n).replace('.','_')}_${size.width}.png`)});}
   }
   ...
   const faded=png(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"})),peak=faded.pixels.reduce((v,n,i)=>i%faded.channels<3?Math.max(v,n):v,0);
