...
async function main(){
  ...
  try{for(const [label,type,renderer] of [['dom',chromium,'dom'],['babylon',chromium,'babylon'],['firefox',firefox,'dom']]){
    ...
    try{
      ...
      const initial=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }",path:path.join(directory,`sky_${label}.png`)}),stats=statistics(initial);
      ...
      const altered=statistics(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"}));
      ...
      assert.equal(compare(initial,await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"})).max,0,'Restoring channel gains reproduces the original pixels');
      await page.evaluate(()=>scenePlayer.setComponent('sky','ProceduralNoise',{enabled:false}));
      assert(statistics(await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"})).std.every(value=>value<.01),'Disabling the component produces a flat sky');
      ...
      const textured=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"});await page.evaluate(()=>scenePlayer.setComponent('sky','ProceduralNoise',{enabled:false}));const plain=await page.screenshot({style:".runtime-loading-status { visibility: hidden !important; }"});
